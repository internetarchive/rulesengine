...
class TreeTestCase(TestCase):
    ...
    def test_surt_only(self):
        results = [str(rule) for rule in
                   tree(Surt('https://(com,example,)/blocked'))]
    ...
    def test_neg_surt(self):
        ...
        results = [str(rule) for rule in
                   tree(
                       Surt('https://(com,example,)/blocked'),
                       neg_surt='https://(')]
        self.assertEqual(results, [])
    ...
    def test_collection(self):
        results = [str(rule) for rule in
                   tree(
                       Surt('https://(com,example,)/collection'),
    ...
    def test_partner(self):
        results = [str(rule) for rule in
                   tree(
                       Surt('https://(com,example,)/partner'),
                       partner='partner')]
        self.assertEqual(
            results, ['BLOCK PLAYBACK (https://(com,example,)/partner)'])
